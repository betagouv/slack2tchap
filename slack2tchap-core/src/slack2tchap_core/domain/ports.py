...
from slack2tchap_core.domain.models import MatrixAccount, User, WebhookEndpoint
...
class SecretCipherPort(Protocol):
    """Port interface for symmetric AEAD encryption / decryption at rest and token payloads."""
    ...
    def encrypt_token(self, plaintext: str) -> str:
        """Encrypt plaintext into a URL-safe self-contained token (nonce + ciphertext + tag)."""
        ...

    def decrypt_token(self, token: str) -> str:
        """Decrypt a URL-safe self-contained token, verifying integrity."""
        ...


class StatelessMessengerPort(Protocol):
    """Port interface for dispatching ephemeral stateless messages to Matrix."""
    ...
    async def send_message(
        ...
        """Send an unencrypted message to a Matrix room via an ephemeral client session."""
        ...
